# Studio GitHub Issues - Draft

## Overview

20 issues across 6 phases for setting up the home studio workflow.

## Studio Layout

```
┌───────────────────────────────────────────────────────────────────────────────┐
│                                                                               │
│   VOCALS WORKSTATION          THE DESK           PRODUCTION        iMAC       │
│   (left)                      (center)           WORKSTATION       STATION    │
│   48ch Patchbay               No patchbay        48ch Patchbay     WSR7500    │
│   Mac Studio                                     MPC XL            Rack       │
│                                                                               │
└───────────────────────────────────────────────────────────────────────────────┘
```

## Stations

### The Desk (center)
- Tascam Mixcast Pro 4
- Shure mic (high-end)
- No patchbay — connects to Vocals workstation via Presonus channel strip

### Vocals Workstation (left)
| Gear | Type |
|------|------|
| Scarlett 18i20 | Interface → Mac Studio (USB) |
| WA73 | Mic Preamp (Mojave → WA73 → 18i20 line in) |
| FET Compressor/Limiter | Dynamics (optional insert) |
| Antares Auto-Tune | Pitch correction (real-time) |
| Klark Teknik EQ | EQ (**possibly broken** - may replace with passive EQ) |
| Presonus Channel Strip | Preamp (Shure mic → Tascam) |
| Mac Studio | Computer (Ableton/Pro Tools) |
| 48ch Patchbay | Routing |

**Signal chain:** Mojave Mic → WA73 → FET Compressor → Auto-Tune → 18i20 LINE input

**Cross-station access:** Via tie lines (4 each direction)

### Production Workstation (right)
| Gear | Type |
|------|------|
| MPC XL | Sampler/DAW |
| Yamaha DGX 670 | Keys (88 weighted) — primary MIDI controller |
| Arturia Minifreak | Synth — sound design → sample into MPC |
| MicroKorg | Vocoder/Synth (rarely used) |
| Numark Mixstream Pro | DJ Controller |
| WA Bus Compressor VCA | Dynamics (creative - Patchbay 2) |
| Pro VLA 2 | Compressor (creative - Patchbay 2) |
| DBX 286s | Channel Strip |
| Stereo Ultragraph Pro EQ | EQ (creative - Patchbay 2) |
| Golden Age MK3 EQ | EQ (creative - Patchbay 2) |
| Lexicon MXL | FX (dual stereo - Patchbay 2) |
| Scarlett OctoPre Dynamic | Interface + compression (seatbelt - Patchbay 1) |
| Steinberg UR-RT4 | Interface (Rupert Neve transformers) |
| Patchbay 1 | Core signal flow (normalled) |
| Patchbay 2 | Creative / Processing (all THRU) |

### iMac Station (WSR7500 Rack)
| Gear | Type |
|------|------|
| 2020 iMac 27" | Computer (Intel, secondary Ableton station) |
| On-Stage WSR7500 | Workstation Rack Cabinet |

**Shared storage:** Ableton projects synced with Mac Studio (iCloud/Dropbox/NAS)

## Files

| File | Issues | Description |
|------|--------|-------------|
| `phase-0-the-desk.md` | #15 | Tascam integration, daily Zoom/work workflow |
| `phase-1-patchbay.md` | #1, #2, #2b, #3, #3b | Patchbay reorganization (both workstations) |
| `phase-2-mpc.md` | #4-5 | MPC as creative center |
| `phase-3-ableton.md` | #6-8, #18 | MPC → Ableton workflow, iMac station setup |
| `phase-4-pro-tools.md` | #9-11 | Ableton → Pro Tools handoff |
| `phase-5-operations.md` | #12-14 | Backup, gain staging, decision checklist |

## Labels to Create

**Phases:**
- `phase-0`, `phase-1`, `phase-2`, `phase-3`, `phase-4`, `phase-5`

**Stations:**
- `the-desk`, `vocals-workstation`, `production-workstation`, `imac-station`

**Gear:**
- `patchbay`, `mpc`, `ableton`, `pro-tools`, `tascam`

**Task Types:**
- `planning`, `hardware`, `documentation`, `template`, `testing`, `workflow`, `setup`, `control`, `backup`, `operations`, `routing`, `ipad`, `critical`

## Patchbay Philosophy

| Patchbay | Purpose | Normalling |
|----------|---------|------------|
| **Patchbay 1** | Core / Default signal flow | NORMALLED |
| **Patchbay 2** | Creative / Processing / FX | ALL THRU |

**If Patchbay 2 is unplugged, your studio still works.**

> **Patchbay 1 = "things I never want to think about."**
> **Patchbay 2 = "things I want to choose."**

- OctoPre Dynamic = **seatbelt** (control, utility)
- VLA, VCA, Lexicon = **paintbrush** (creative choices)

## Mental Model

```
MPC (ideas, groove, feel)
    ↓
Ableton (exploration, arrangement, performance)
    ↓
Pro Tools (commitment, vocals, polish)
```

## Dependencies

```
Phase 1 (Patchbay):
#1 → #2 (Vocals) → #3
  → #2b (Production) ↗
                     ↓
                     #5 (MPC verification)
                     #15 (Tascam integration)

#3b (Klark EQ test) — no dependencies

Phase 3 (Ableton):
#6 → #7
  → #8

Phase 4 (Pro Tools):
#9 → #10
```

## Critical Path

**Priority 1 (daily workflow):** #1 → #2 → #15 (Tascam/Zoom setup)
**Priority 2 (creative foundation):** #2b → #4 → #5 (MPC + patchbay)
**Priority 3 (DAW integration):** #6 → #7 → #9 → #10 (Ableton + Pro Tools)
